...
import de.thb.ue.android.activities.EnlargeImageActivity;
import de.thb.ue.android.custom.CustomEditText;
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        view.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                TextQuestionDTO dto = DataHolder.retrieveTextQuestionDTO(mQuestion);
...
//        EventBus.getEventBus().post(new FragmentGotPrimaryEvent(mQuestion, mImageName));
    ...
    public void onLeavingPrimary() {
        ...
        TextAnswerDTO answerDTO = DataHolder.isTextQuestionAnswered(mQuestion);
        if(answerDTO == null){
            DataHolder.getAnswersDTO().getTextAnswers().add(new TextAnswerDTO(mQuestionID, mQuestion, answer));
        } else {
    ...
    public void onPhotoTaken(String question, String imagePath){
        if(/*mCurrentlyPrimary &&*/ question.equals(mQuestion)){
            ImagePathsVO pathObj = DataHolder.getCommentaryImageMap().put(mQuestion, new ImagePathsVO(imagePath, null, null));
...
//            Picasso.with(getActivity()).setLoggingEnabled(true);
    ...
    public void onReloadImage(){
        if( DataHolder.getCommentaryImageMap().containsKey(mQuestion)){
            Utility.animateView(mProgressBar, View.VISIBLE, 1.0f, 100);
            Utility.animateView(mImageView, View.INVISIBLE, 0, 100);
            ImagePathsVO pathObj = DataHolder.getCommentaryImageMap().get(mQuestion);
            Picasso.with(getActivity()).cancelRequest(mImageView);
            Picasso.with(getActivity())
